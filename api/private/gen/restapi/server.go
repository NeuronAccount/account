...
// NewServer creates a new api accounts server but does not configure it
func NewServer(api *operations.AccountsAPI) *Server {
	s := new(Server)
...
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/accounts.sock"`
...
func (s *Server) SetAPI(api *operations.AccountsAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(1)
		s.Logf("Serving accounts at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer wg.Done()
			if err := domainSocket.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving accounts at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	}
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(1)
		s.Logf("Serving accounts at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving accounts at http://%s", l.Addr())
		}(s.httpServerL)
	}
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(1)
		s.Logf("Serving accounts at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpsServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving accounts at https://%s", l.Addr())
