...
// NewServer creates a new api account private server but does not configure it
func NewServer(api *operations.AccountPrivateAPI) *Server {
	s := new(Server)
...
// Server for the account private API
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/account-private.sock"`
	...
	api          *operations.AccountPrivateAPI
...
func (s *Server) SetAPI(api *operations.AccountPrivateAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(1)
		s.Logf("Serving account private at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer wg.Done()
			if err := domainSocket.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving account private at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	}
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(1)
		s.Logf("Serving account private at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving account private at http://%s", l.Addr())
		}(s.httpServerL)
	}
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(1)
		s.Logf("Serving account private at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpsServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving account private at https://%s", l.Addr())
